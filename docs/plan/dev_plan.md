...
| **Task 11** | Debate Subgraph 构建      | 是    | `app/graph/polishing/debate_graph.py`                                                                                      | 1. 定义 Author-Editor 对抗循环<br>2. 配置条件边（score >= 90 或 iteration >= 3 结束）<br>3. 编译子图并导出                                                                                                                                    | 150 | `langgraph`                                                                                           |
| **Task 12** | Polishing Graph 构建      | 是    | `app/graph/polishing/builder.py`, `app/graph/polishing/debate/`                                                               | 1. Debate 子图独立目录（debate/state.py, debate/prompts.py, debate/nodes.py, debate/builder.py）<br>2. 主图通过 debate_node 包装节点调用编译后的 Debate Subgraph<br>3. PolishingState ↔ DebateState 状态映射<br>4. 三档模式路由（Mode 1/2/3）<br>5. 编译图并返回单例 | 300 | `langgraph`, `app.graph.polishing.debate`                                                             |
| **Task 13** | Checkpointer 与服务层       | 是    | `app/services/checkpointer.py`, `app/services/creation_svc.py`, `app/services/polishing_svc.py`                            | 1. 实现 Checkpointer 单例（开发用 MemorySaver，生产用 PostgresSaver）<br>2. 封装 Creation 业务逻辑（启动、恢复、状态查询）<br>3. 封装 Polishing 业务逻辑<br>4. Graph Builder 改造支持 Checkpointer 注入<br>5. 使用 GraphInterrupt 替代已弃用的 NodeInterrupt              | 400 | `langgraph-checkpoint`, `langgraph-checkpoint-postgres`                                               |
| **Task 14** | FastAPI 路由层             | 是    | `app/api/dependencies.py`, `app/api/v1/creation.py`, `app/api/v1/polishing.py`, `app/api/v1/router.py`                     | 1. 实现依赖注入（Graph 实例、Checkpointer）<br>2. 实现 Creation 路由（POST /creation, GET /tasks/{id}, POST /tasks/{id}/resume）<br>3. 实现 Polishing 路由（POST /polishing）<br>4. 聚合路由到 v1                                                  | 350 | `fastapi`                                                                                             |
| **Task 15** | 应用入口与生命周期               | 是    | `app/main.py`                                                                                                              | 1. 创建 FastAPI 应用实例<br>2. 挂载 CORS 中间件<br>3. 注册全局异常处理器<br>4. 实现 startup 事件（初始化 Checkpointer、LLM）<br>5. 实现 shutdown 事件（清理资源）<br>6. 注册 v1 路由                                                                               | 150 | `fastapi`, `uvicorn`                                                                                  |
| **Task 16** | 启动应用                    | 是    |      `app/main.py`                     | 1.在虚拟环境中运行 `uvicorn app.main:app` 启动应用， 访问 http://localhost:8000/docs 查看 API 文档。 2.访问所有接口，并测试功能。确保所有功能正常。                                                                                                              | 0  | 无                                                                                                     |
