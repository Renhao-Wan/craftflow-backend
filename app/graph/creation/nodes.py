...
def should_continue_writing(state: CreationState) -> str:
    """条件边函数：判断是否需要继续撰写章节
    ...
    # 如果还有未撰写的章节，继续调用 WriterNode
    if len(sections) < len(outline):
        logger.info(f"还有 {len(outline) - len(sections)} 个章节待撰写")
        return "writer"

    # 所有章节完成，进入 ReducerNode
    logger.info("所有章节已完成，进入合并阶段")
    return "reducer"


def should_end_or_continue(state: CreationState) -> str:
    """条件边函数：判断是否应该结束或继续
