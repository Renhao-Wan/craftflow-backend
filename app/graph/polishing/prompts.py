...
AuthorNode 和 EditorNode 的 Prompt 模板位于 debate/prompts.py。
"""
...
**重要指令**：
1. 你必须使用搜索工具（search_internet 或 search_with_answer）来验证文章中的关键事实
2. 不要仅凭你的知识判断，必须通过搜索获取最新信息
3. 至少搜索 2-3 次来验证不同的事实点
4. 搜索完成后，以 JSON 格式返回核查结果"""
